...
// creating next and prev buttons
...
//Checking wether on the first song and setting Url
	if(!pages[Npg - 1]){
		next = pages[Npg + 1]+".html";
		prev = pages[pages.length - 1]+".html";
		//console.log("yes");
	}
//Checking wether on the last song and setting Url
	else if(!pages[Npg + 1]){
		next = pages[0]+".html";
		prev = pages[Npg - 1]+".html";
...
tT /= 1000
	if (tT > 0.3){
		...
		if(startingX+250 < movingX){
			// console.log('right');
			B4A.CallSub('swipeRight', true)
		} 
		else if(startingX-250 > movingX){
			// console.log('left');
			B4A.CallSub('swipeLeft', true)
		}
