...
public class err extends Activity implements B4AActivity{
	public static err mostCurrent;
	...
	public void onCreate(Bundle savedInstanceState) {
        ...
        mostCurrent = this;
		if (processBA == null) {
			processBA = new BA(this.getApplicationContext(), null, null, "com.LugandaHymnalNew", "com.LugandaHymnalNew.err");
			...
			if (p != null && p != this) {
                BA.LogInfo("Killing previous instance (err).");
				p.finish();
			}
		}
	...
	private void afterFirstLayout() {
        if (this != mostCurrent)
			return;
		activityBA = new BA(this, layout, processBA, "com.LugandaHymnalNew", "com.LugandaHymnalNew.err");
        ...
        if (BA.isShellModeRuntimeCheck(processBA)) {
			...
			processBA.raiseEvent2(null, true, "CREATE", true, "com.LugandaHymnalNew.err", processBA, activityBA, _activity, anywheresoftware.b4a.keywords.Common.Density, mostCurrent);
			_activity.reinitializeForShell(activityBA, "activity");
		}
        initializeProcessGlobals();		
        initializeGlobals();
        
        BA.LogInfo("** Activity (err) Create " + (isFirst ? "(first time)" : "") + " **");
        processBA.raiseEvent2(null, true, "activity_create", false, isFirst);
		isFirst = false;
		if (this != mostCurrent)
			return;
        processBA.setActivityPaused(false);
        BA.LogInfo("** Activity (err) Resume **");
...
@Override
 ...
 protected void onStop() {
	...
	public void onPause() {
        ...
        if (!dontPause)
            BA.LogInfo("** Activity (err) Pause, UserClosed = " + activityBA.activity.isFinishing() + " **");
        else
            BA.LogInfo("** Activity (err) Pause event (activity is not paused). **");
        if (mostCurrent != null)
...
public static String  _activity_create(boolean _firsttime) throws Exception{
 ...
 //BA.debugLineNum = 25;BA.debugLine="Activity.LoadLayout(\"error.bal\")";
mostCurrent._activity.LoadLayout("error.bal",mostCurrent.activityBA);
